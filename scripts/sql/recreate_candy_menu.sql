-- =============================================
-- 清理重复菜单并重新创建Candy AI菜单结构
-- 设置为4个菜单：用户管理、角色管理、对话管理、系统设置
-- =============================================

-- 1. 清理重复的Candy AI菜单和相关数据
DELETE FROM sys_role_menu WHERE menu_id IN (4, 2000, 400, 401, 402, 4000, 4001, 4002, 4003, 4004, 4005, 4006, 4007, 4010, 4011, 4012, 4013, 4014, 4020, 4021, 4022);
DELETE FROM sys_menu WHERE menu_id IN (4, 2000, 400, 401, 402, 4000, 4001, 4002, 4003, 4004, 4005, 4006, 4007, 4010, 4011, 4012, 4013, 4014, 4020, 4021, 4022);
...
(1, 402), -- 对话管理
(1, 403), -- 系统设置
(1, 4000), (1, 4001), (1, 4002), (1, 4003), (1, 4004), (1, 4005), -- 用户管理按钮
(1, 4010), (1, 4011), (1, 4012), (1, 4013), (1, 4014), -- 角色管理按钮
(1, 4020), (1, 4021), (1, 4022), -- 对话管理按钮
(1, 4030), (1, 4031); -- 系统设置按钮 